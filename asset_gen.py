...
added content by me (choysal):
adding support for additional player sprite + 2 additional sprites

collaboration note: 
Mandy Hu (mandyh) and I (choysal) worked together on adding 
the additional player and additional sprite support.
But our games are different, the changes are probably not the exact same.
However, the concept is similar.

Usage:
    python asset_gen.py background.png player.png sprite1.png sprite2.png
...
def find_tiles_and_palettes(img: np.ndarray, player: np.ndarray, bad_apple: np.ndarray, good_apple: np.ndarray):
    print(img.shape)
    assert tuple(img.shape) == bg_size, "Image must be 240x256x4 for this script"
    assert tuple(player.shape) == sprite_size, "sprite must be 8x8x4 for this script"
    assert tuple(bad_apple.shape) == sprite_size, "sprite must be 8x8x4 for this script"
    assert tuple(good_apple.shape) == sprite_size, "sprite must be 8x8x4 for this script"
    ## --- Tiles ---
    ## read bg
    tiles = {}
    ...
    ## read player sprite
    for y in range(0, sprite_size[0], 8):
        for x in range(0, sprite_size[1], 8):
            tile = player[y : y + 8, x : x + 8, :]
            assert tuple(tile.shape) == (8, 8, 4)

            if tile.tobytes() not in tiles.keys():
                tiles[tile.tobytes()] = tile

    ## read bad apple sprite
    for y in range(0, sprite_size[0], 8):
        for x in range(0, sprite_size[1], 8):
            tile = bad_apple[y : y + 8, x : x + 8, :]
            assert tuple(tile.shape) == (8, 8, 4)

            if tile.tobytes() not in tiles.keys():
                tiles[tile.tobytes()] = tile

    ## read good apple sprite
    for y in range(0, sprite_size[0], 8):
        for x in range(0, sprite_size[1], 8):
            tile = good_apple[y : y + 8, x : x + 8, :]
            assert tuple(tile.shape) == (8, 8, 4)
...
parser.add_argument("bad_apple")
parser.add_argument("good_apple")
args = parser.parse_args()
img = load_image(args.image)
player = load_image(args.player)
bad_apple = load_image(args.bad_apple)
good_apple = load_image(args.good_apple)
find_tiles_and_palettes(img,player,bad_apple,good_apple)
